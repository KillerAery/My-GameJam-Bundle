...
public class ThirdPersonULCon : MonoBehaviour
{
	//记录--------------------------------------------------------
	...
	void speedUpdateSet()//在Update中设置速度
	{
		...
		{
			baseDir = movetoPos;
			cmabaseTrans.position = new Vector3(mycamera.transform.position.x, mycamera.transform.position.y - mycamera.transform.localPosition.y, mycamera.transform.position.z);
	...
	void cmaDirUpdateSet()//根据鼠标设置Cma的位置
	{
		...
		if (!Cursor.visible)//鼠标不可见时
		{
			...
			//完成Player x_横向旋转
			mycamera.transform.RotateAround(gameObject.transform.position, gameObject.transform.up, rotate_x);
			if ((nowAngle_y + rotate_y) > 30f || nowAngle_y + rotate_y < -30f)
			{
				return;
			}
			nowAngle_y += rotate_y;
			...
			{
				mycamera.transform.localPosition = new Vector3(mycamera.transform.localPosition.x, -0.75f + 3.75f * ((nowAngle_y + 30f) / 60f), mycamera.transform.localPosition.z);
			}
			else
			{
				mycamera.transform.localPosition = new Vector3(mycamera.transform.localPosition.x, -0.75f * ((nowAngle_y + 90f) / 60f), mycamera.transform.localPosition.z);
			}
			...
			//if (nowAngle_y > -30f && nowAngle_y <= 30f)
			//{
			//	onConCma.transform.localPosition = new Vector3(onConCma.transform.localPosition.x, -0.75f + 3.75f * ((nowAngle_y + 30f) / 60f), -5f + 1f * ((nowAngle_y + 30f) / 60f));
			//}
			//else
			//{
			//	onConCma.transform.localPosition = new Vector3(onConCma.transform.localPosition.x, -0.75f * ((nowAngle_y + 90f) / 60f), -0.75f - 4.25f * ((nowAngle_y + 90f) / 60f));
			//}

			//if (nowAngle_y > -60f && nowAngle_y <= 30f)
			//{
			//	onConCma.transform.localPosition = new Vector3(0, -2f + 5f * ((nowAngle_y + 60f) / 90f), -3f - 1f * ((nowAngle_y + 60f) / 90f));
			//}
			//else
			//{
			//	onConCma.transform.localPosition = new Vector3(0, -2f * ((nowAngle_y + 90f) / 30f), -0.75f - 3.25f * ((nowAngle_y + 90f) / 30f));
			//}
		}
		else
		{
	...
	void shelterCheckUpdate()//遮挡透明化处理
	{
		...
		basePos2 = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y - 1f, gameObject.transform.position.z);
